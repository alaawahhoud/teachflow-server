...
/** يحجز أول page_id فاضي (1..MAX_SLOT) اعتماداً على users.fingerprint_page_id */
...
router.post("/enroll-request", async (req, res) => {
  ...
  try {
    ...
    await conn.query(
      ...
      [userId, String(device_id), pageId]
    );

    await conn.commit();
    return res.json({ user_id: String(userId), device_id: String(device_id), pageId });
  } catch (e) {
    await conn.rollback();
    console.error("[/api/fingerprints/enroll-request]", e);
...
router.get("/command", async (req, res) => {
  ...
  try {
    const [rows] = await pool.query(
      "SELECT user_id, page_id FROM fp_enroll_queue WHERE device_id=? AND status='pending' " +
    ...
    console.error("[/api/fingerprints/command]", e);
    return res.status(500).json({ action: "none" });
  }
});

/* =============== 3) ESP32 يبلغ بنتيجة أو match =============== */
  ...
  try {
    if (status === "enroll_ok") {
      const [qrows] = await pool.query(
        "SELECT id, user_id FROM fp_enroll_queue WHERE device_id=? AND page_id=? AND status='pending' " +
        "ORDER BY created_at DESC LIMIT 1",
        [String(deviceId), Number(pageId)]
      );
      if (!qrows.length) {
    ...
    if (status === "match") {
      if (!pageId) return res.status(400).json({ message: "pageId required for match" });
      ...
      // منع التكرار خلال 5 دقائق (created_at)
      ...
      return res.json({
        ok: true, action: "match", user_id: String(uid), pageId: Number(pageId),
    ...
    console.error("[/api/fingerprints/scan]", e);
    return res.status(500).json({ message: "scan failed" });
  }
});

/* =============== 4) حالة التسجيل للواجهة =============== */
router.get("/enroll-status", async (req, res) => {
  ...
  try {
    ...
    return res.json({
      ...
      pageId: Number(st.page_id),
      status: st.status,
      note: st.note
    });
  } catch (e) {
    console.error("[/api/fingerprints/enroll-status]", e);
...
router.get("/_debug/state", async (_req, res) => {
  ...
  const [att]   = await pool.query("SELECT * FROM attendance ORDER BY created_at DESC LIMIT 50");
